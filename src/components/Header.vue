<template>
  <header class="bg-black shadow-md fixed top-0 left-0 right-0 z-50">
    <div class="flex justify-between items-center h-16 px-4 md:px-12 w-full">
      ...
      <div class="flex items-center gap-4 ml-auto">
        ...
        <nav class="hidden md:flex gap-6 items-center text-white font-medium">
          <router-link to="/" class="hover:text-gray-300">Home</router-link>
          <router-link to="/shop" class="hover:text-gray-300">Products</router-link>
        ...
        <router-link v-else to="/login" class="bg-white text-black font-semibold py-1.5 px-4 rounded hover:bg-gray-200 transition duration-200">Login</router-link>

        <template v-if="!isHomePage">
          <router-link to="/wishlist" title="Wishlist">
            <img src="/download.png" alt="Wishlist" class="h-10 w-10 cursor-pointer hover:opacity-80 transition -ml-1.8" />
          </router-link>

          <router-link to="/cart" title="Cart">
            <img src="/download (1).png" alt="Cart" class="h-10 w-10 cursor-pointer hover:opacity-80 transition -ml-2" />
          </router-link>

          <router-link v-if="isAuthenticated" to="/orders" class="bg-white text-black font-semibold py-1.5 px-4 rounded hover:bg-gray-200 transition duration-200">My Orders</router-link>
        </template>
      </div>
...
const doLogout = () => { logout(); showDropdown.value = false }
</script>
